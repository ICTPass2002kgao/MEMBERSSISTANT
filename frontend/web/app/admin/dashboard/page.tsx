...
// Added your custom API and SharedUI imports
...
export default function SystemAdminDashboard() {
    ...
    type LandlordStatus = { name: string; value: number; color: string };
    const [landlordStatusData, setLandlordStatusData] = useState<LandlordStatus[]>([]);
    
    type UserDemographic = { name: string; count: number; fill: string };
    ...
    const [recentLandlords, setRecentLandlords] = useState<Landlord[]>([]);

    useEffect(() => {
        const fetchSystemData = async () => {
            try {
                // Utilizing apiFetch to automatically handle tokens, BASE_URL, and 401/403 redirects
                // Added .catch(() => []) to prevent the entire dashboard from breaking if one endpoint fails
                ...
                const pendingIssues = issuesList.filter((i: { status: string }) => i.status !== 'RESOLVED').length;
                ...
                landlordsList.forEach((l: Landlord) => {
    ...
    // Matched loading state styling to AccommodationsPage
    if (loading) return <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-blue-500" size={40} /></div>;

    return (
        // Matched animation duration and slide direction to AccommodationsPage
...
function StatCard({ title, value, label, accentColor, accentBg, iconColor, icon: Icon }: StatCardProps) {
    return (
