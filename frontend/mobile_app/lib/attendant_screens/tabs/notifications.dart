...
class AttendantNotifications extends StatefulWidget {
  const AttendantNotifications({super.key});

  @override
  State<AttendantNotifications> createState() => _AttendantNotificationsState();
}

class _AttendantNotificationsState extends State<AttendantNotifications> {
  ...
  void initState() {
    ...
    // Listen to the reusable service!
    _notificationSubscription = PushNotificationService().onMessageStream.listen((message) {
      if (mounted) {
        // A new push just arrived! Refresh the list from Django.
        _fetchNotifications();
      }
    }); 
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            // Sort by most recent transmission
            _notifications.sort(
              (a, b) =>
        ...
        throw Exception('Failed to synchronize with Command Center.');
      }
    } catch (e) {
      if (mounted)
        setState(() {
  ...
  Future<void> _markAsRead(int index, String id) async {
    ...
    try {
      ...
      debugPrint("Read-receipt sync failed: $e");
    }
  }
