...
class ResponderFaceVerificationScreen extends StatefulWidget {
  final Map<String, dynamic> userData;

  const ResponderFaceVerificationScreen({
    super.key,
    required this.userData,
  });

  @override
  State<ResponderFaceVerificationScreen> createState() =>
      _ResponderFaceVerificationScreenState();
}

class _ResponderFaceVerificationScreenState
    extends State<ResponderFaceVerificationScreen>
  ...
  Future<void> _processBackendUpload(Uint8List capturedBytes) async {
    setState(() {
      _processStatus = "Verifying dispatch clearance...";
    });
    ...
    try {
      ...
      var request = http.MultipartRequest(
        'POST',
        Uri.parse(
          '${ApiClass().getApiBaseUrl()}/verify-responder-login/',
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        _showMessage("Clearance verified! Accessing Dispatch.", Colors.green);

        // --- SUCCESS: Navigate to Staff Main Menu ---
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const StaffMainMenu()),
        );
        ...
        _handleFailure(reason: "Identity unverified.");
      } else {
        final error =
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          const BubbleBackground(), // Reused from login.dart
          SafeArea(
            child: Center(
                  ...
                  child: Column(
                    children: [
                      ...
                      if (!_hasAgreedToDisclaimer)
                        _buildDisclaimerPanel(textColor, Colors.redAccent)
                      else if (_isVerifyingBackend)
                        _buildProcessingPanel(textColor, Colors.redAccent)
                      else
                        _buildLiveCamPanel(textColor, Colors.redAccent, theme),
                    ],
                  ),
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: GoogleFonts.poppins(color: hintColor, fontSize: 12, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
    ...
    return _glassContainer(
      child: Column(
        ...
        children: [
          ...
          Text(
            "Access to the Medical Dispatch dashboard requires live biometric authentication. This action is logged securely.",
            style: GoogleFonts.poppins(
              fontSize: 14,
